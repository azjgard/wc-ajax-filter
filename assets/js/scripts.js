...
// This function will be called on the initial page
// load and after every time the results are filtered.
// It removes all of the descendent filter options
// whose parents are not selected.
function removeUnchosenChildren($, data) {
    ...
    lis.each(function() {
        ...
        var grandparentClass = $(grandparent).attr('class') || '';

        var isChild         = parentClass.includes('children');
        var isUnchosenChild = !grandparentClass.includes('chosen');

        var siblings = $(this).siblings();

        console.log(siblings);

        // If the <li>'s parent has a class of children,
        // AND the <li>'s grandparent does NOT have a class of chosen,
        // then remove the element.
        if (isChild && isUnchosenChild) {
            $(this).remove();
        }
    });
}
...
jQuery(document).ready(function($) {
    ...
    // Hide the right list items.
    removeUnchosenChildren($, $('.wcapf-layered-nav'));
    ...
    wcapfFilterProducts = function() {
        ...
        // this sends a get request to the URL with the query string,
        // which returns the updated data. it then puts that updated data
        // in the box. This means that the selection options are actually
        // taken out the in PHP somewhere. I'll find where and then put
        // the name of the file here:
        //
        // filename: functions.php
        //
        // What I need to do is instead of choosing to not return the html
        // for the elements that don't match the filter, return them, but
        // have them grayed out and unselectable.
        $.get(window.location.href, function(data) {
            ...
            $.each(widgets, function(index, id) {
                ...
                removeUnchosenChildren($, single_widget);
